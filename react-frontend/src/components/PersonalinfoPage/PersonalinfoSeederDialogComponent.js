...
import seederData from "./personalinfo.seeder.json";
...
const PersonalinfoSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("personalinfo").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(PersonalinfoSeederDialogComponent);
