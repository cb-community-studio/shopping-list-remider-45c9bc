...
import seederData from "./shoppinglist.seeder.json";
...
const ShoppinglistSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("shoppinglist").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ShoppinglistSeederDialogComponent);
